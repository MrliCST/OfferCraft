...
## 12. 全量爬取的韧性加固（2026-09-19 深夜 ~ 09-20）

老板要求「直接全量爬取」，实际连吃三记闷棍。三者的成因各不相同，且**第三记是我自己
「想让它更稳」而制造出来的**，值得完整记下来。

### 12.1 第一记：无限等待（静默挂死 13.5 小时）

**症状**：进程活着，但 CPU 累计仅 11 秒、29 个线程全在睡、网络连接为 0，
日志停在进入第一个栏目之后，`ps etime` 显示已经 13 小时 28 分。

**根因**：`CrawlThrottle.beforeCrawl` 用 `Semaphore.acquireUninterruptibly()`，**没有超时**。
许可只在 `afterCrawl` 归还，而调用方是在 `finally` 里调的 —— 只要代码能走到 `finally` 就没事。
但 Playwright 的 `navigate` 在页面永久白屏 / 连接被中间设备黑洞时**不返回**，
于是 `finally` 也到不了，许可永远拿不回来。信号量只有 2 个许可，
两篇卡死的帖子就把它占满，**后续所有帖子全部在 `beforeCrawl` 上静默阻塞**。

**修法（两道防线，缺一不可）**：
1. `beforeCrawl` 改 `tryAcquire(120, SECONDS)`，拿不到就抛异常。
   调用方本来就在 try/catch 里抓单篇失败，一篇卡死不该拖垮整个爬取；
2. 给导航设显式超时 —— `dp.setDefaultNavigationTimeout(15s)`（详情页）、
   `page.setDefaultNavigationTimeout(30s)`（feed）。
   **这是根本手段：超时才是防卡死的正解，前面那条只是让故障可见。**

### 12.2 第二记：浏览器进程崩溃（TargetClosedError）

**症状**：API 翻页到第 33 页时 Chrome 整个没了，异常直接杀掉 main 线程，
前面爬到的数据全丢在内存里。日志 `API 索引: 609 篇`（本该 40 页 ≈ 761 篇）。

**根因**：长跑中 Chrome 自身不稳定；且主循环**没有止损逻辑** —— 浏览器已经死了，
代码还继续拿死掉的 page 去 `clickChip`，于是 `TargetClosedError` 终止整个进程。

**修法**：
1. 主循环每栏目包 try/catch，用 `isTargetClosed()` 分辨「环境没了」还是「单篇有问题」。
   前者 `break` 止损（后面所有栏目都无从谈起），后者只记一条失败继续；
2. 栏目体抽成独立方法 `crawlColumn(...)`，让「失败边界」在代码里看得见；
3. **增量落盘**：每 50 篇写一次盘 —— 崩溃时最多丢 50 篇，不用整栏重来；
4. 栏目崩溃时先 `saveColumn(...)` 再往外抛，保住已爬到的。

### 12.3 第三记：Chrome 首次导航就崩 —— 我自己造的

**症状**：连一条 `[API]` 都没有，`navigating to ".../group/51121244585524"` 直接 TargetClosed。
此前同样的代码（只换了启动参数）是能跑的。

**根因**：我为「加固」给 Chrome 加了 `--js-flags=--max-old-space-size=1024`。
知识星球是重 SPA，**1GB 的 V8 堆上限直接被撑爆，渲染进程当场崩溃**。
`--disable-background-networking` 顺带掐掉了 SPA 的资源预取。

顺带也撤掉了更早加的 `--disable-dev-shm-usage`：本机 `/dev/shm` 有 12G，
强制走磁盘临时文件反而更不稳。

**教训（这条要单独立碑）**：

> **不要给 Chrome 加「看起来更省资源」的限制。**
> 抓 SPA 时这些限制会把页面搞崩，而崩溃的代价远大于多占点内存。
> 我因为「想让它更稳」反而亲手制造了一次崩溃。

现在只保留三件必需的：`--no-sandbox`（无头环境）、`--disable-gpu`、`--disable-extensions`。

### 12.4 一个骗过我的编译陷阱

第 12.2 节的 `isTargetClosed()` 里我写了：
```java
t instanceof com.microsoft.playwright.impl.PlaywrightException
```
这是 Playwright 的**内部实现类**，classpath 上取不到。

问题在于 **`mvn -o -q compile` 报了 EXIT=0，却是假的** ——
增量编译认为这个类没变就没重新编，`target/classes` 里还留着上一版能跑的 class。
直到运行期才抛 `java.lang.Error: Unresolved compilation problem`。

**修法**：判据改落在**消息文本 + 类名**上（`getClass().getSimpleName().contains("TargetClosed")`
或消息含 `"Target page, context or browser has been closed"`），不直接 catch 那个类型 ——
Playwright 公开 API 里本来就没有对应的可 catch 类型。

**附带教训**：改完涉及「编译期不可见类型」的代码后，**必须 `mvn -o clean compile`**，
不能用 `-q compile` 的结果当验收依据。

### 12.5 修复后验收

| 项 | 结果 |
|---|---|
| `zsxq-smoke3b`（starmaster 3 篇） | 正文 **14313 / 24971 / 16350 字**，图 4/11/4 张 |
| 血缘键 / 文章页链接 | 齐全，与历史验收完全一致 |
| API 索引 | **761 篇**（40 页拉满）；此前两次为 77 / 609 篇 |
| 首屏导航失败 | 打印「浏览器是否可用 / 登录态是否过期」，不再抛堆栈 |

**已知小瑕疵**：`topics` 接口偶发返回空页（HTTP 200 但 `topics: []`），重试一次仍空就
`break`。所以索引篇数会在 134 ~ 761 之间浮动。这只影响血缘键补全率，不影响正文抓取。

### 12.6 第四记：Chrome 无规律崩溃 → 改为「崩了就重启」

第 12.2 节的止损修好之后（故障可见了），跑全量仍在**不同位置**崩：一次在第 33 页翻页、
一次在第 3 页、一次在第 4 页。**没有任何稳定的触发点**。

**排查（逐一排除）**：

| 假设 | 验证 | 结论 |
|---|---|---|
| 内存 / `/dev/shm` 不足 | `free -h` 9.7G 可用、`/dev/shm` 12G | ✗ 不是 |
| Chrome 版本与 Playwright 不兼容 | 最小复现：Chrome 151 + Playwright 1.57 启动、连续开 5 页导航 | ✗ 都正常 |
| 站点反爬 / 登录态失效 | 真实站点探测：HTTP 200、标题正确、20 个帖子元素、连续 5 页 OK | ✗ 不是 |

基础能力全绿，但真实爬取无规律崩 —— 说明是**长跑累积的不稳定**（页面泄漏、
`context.request()` 与页面导航混用、SPA 长时间驻留），而不是某个可修的具体 bug。

**结论：与其继续追查，不如让爬虫对崩溃免疫。** 全量爬取要跑上千篇、耗时以小时计，
「崩一次就整体中止」本身就是设计缺陷 —— 无论崩溃根因是什么，这个缺陷都要修。

**改法**：
1. **浏览器崩溃 → 原地重启接着爬**（每个栏目最多 `BROWSER_RESTART_LIMIT = 2` 次）。
   非 `TargetClosed` 的异常不重启（重启也没用）；
2. **API 索引降级为可选**：它是「锦上添花」不是「必需品」（只为补血缘键），
   拉失败就退化成「星友帖没有 postId / sourceUrl」，正文照样爬。
   之前它一旦失败就带走整个爬取，风险收益完全不成比例。

**为什么重启是合理的**：崩溃点是随机的，重启后从**当前栏目的开头**重来
（该栏目已爬到的帖子因为增量落盘还在），代价最多是重爬一栏。相比「整个爬取作废」，
这个代价可以接受。
